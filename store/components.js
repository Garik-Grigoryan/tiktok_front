...
export const actions = {
  async fetch({commit}){
    let components = await this.$axios.$get('http://tiktokback.neoteric-software.com/api/components/get');
    commit('setComponents', components);
  },
  async getComponent({commit}, [id]){
    let component = await this.$axios.$get('http://tiktokback.neoteric-software.com/api/components/get/' + id);
    if(id == 1){
  ...
  await this.$axios.$put('http://tiktokback.neoteric-software.com/api/components/update/'+id, { data });
  return true;
},
