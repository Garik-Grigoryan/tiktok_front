    ...
    >
      ...
      <v-list class="mt-5">
        <v-list-item class="mt-5">
          <v-range-slider v-model="range" :max="max" :min="min" hide-details class="align-center" thumb-label="always" color="#504f4f" track-color="#303030" @change="filter($event)" >
          </v-range-slider>
        </v-list-item>
        <v-list-item v-for="item in items" :key="item.title" >
          <v-list-item-content>
            ...
            <v-list-item-title v-if="$i18n.locale === 'en'">{{ item.title }}</v-list-item-title>
              ...
              <!-- <v-combobox v-else @change="filter($event)" v-model="item.select" :items="item.data" label="" dense chips small-chips multiple ></v-combobox> -->
  ...
  export default {
    ...
    methods: {
      filter(e) {
        ...
        this.$store.dispatch('products/Filter', [properties, this.range, this.$route.params.id]);
